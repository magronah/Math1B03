...
\hypertarget{lecture-2-1}{%
\chapter{Lecture 2}\label{lecture-2-1}}
...
\hypertarget{inverse-of-matrices}{%
\chapter{Inverse of Matrices}\label{inverse-of-matrices}}
...
\tightlist
\item
  Write the augmented matrix \((A|I_n)\), where \(I_n\) is the \(n \times n\) identity matrix.
\item
  Use row operations to transform \((A|I_n)\) into the form \((I_n|B)\), where \(B\) is the inverse of \(A\).
\item
  If \((A|I_n)\) cannot be transformed into \((I_n|B)\), then \(A\) is not invertible.
\end{enumerate}
...
\hypertarget{properties-of-invertible-matrices}{%
\section{Properties of Invertible Matrices}\label{properties-of-invertible-matrices}}
...
\hypertarget{important-theorems}{%
\section{Important Theorems}\label{important-theorems}}
...
\item
  The linear transformation \(T(x)=Ax\) maps \(R^n\) onto \(R^n\).
\item
  There exist an \(n\times n\) inverse matrix \(C\) such that \(CA= I\) and \(AC= I\) .
\end{enumerate}
...
\chapter{Invertible Matrices}\label{invertible-matrices}}
